...
class EncomendaSerializer(serializers.ModelSerializer):
    # Campos adicionais de leitura
    cadastrado_por_username = serializers.CharField(
        source="cadastrado_por.username", read_only=True
    )
    entregue_por_username = serializers.CharField(
        source="entregue_por.username", read_only=True
    )

    class Meta:
        model = Encomenda
        fields = "__all__"  # mantém todos os campos
        read_only_fields = [
            "cadastrado_por", 
            "entregue_por", 
            "data_chegada", 
